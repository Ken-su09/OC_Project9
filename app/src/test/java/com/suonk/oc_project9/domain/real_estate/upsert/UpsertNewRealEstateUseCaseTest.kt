...
class UpsertNewRealEstateUseCaseTest {
    ...
    @Test
    fun `try to invoke`() = testCoroutineRule.runTest {
        // WHEN
        coEvery { realEstateRepository.upsertRealEstate(getDefaultRealEstateEntity()) } returns DEFAULT_ID
        coJustRun { photoRepository.insertPhoto(PhotoEntity(0, DEFAULT_ID, getDefaultAggregatedPhotos()[0].uri)) }
        coJustRun { photoRepository.insertPhoto(PhotoEntity(0, DEFAULT_ID, getDefaultAggregatedPhotos()[1].uri)) }
        coJustRun { photoRepository.insertPhoto(PhotoEntity(0, DEFAULT_ID, getDefaultAggregatedPhotos()[2].uri)) }

        upsertNewRealEstateUseCase.invoke(getDefaultRealEstateEntity(), getDefaultAggregatedPhotos())

        coVerify {
            realEstateRepository.upsertRealEstate(getDefaultRealEstateEntity())
            photoRepository.insertPhoto(PhotoEntity(0, DEFAULT_ID, getDefaultAggregatedPhotos()[0].uri))
            photoRepository.insertPhoto(PhotoEntity(0, DEFAULT_ID, getDefaultAggregatedPhotos()[1].uri))
            photoRepository.insertPhoto(PhotoEntity(0, DEFAULT_ID, getDefaultAggregatedPhotos()[2].uri))
        }
