...
class MapViewModelTest {
    ...
    private companion object {
        private const val DEFAULT_LAT_INITIAL_CASE = 0.0
        private const val DEFAULT_LNG_INITIAL_CASE = 0.0
        private val DEFAULT_POSITION_INITIAL_CASE = Position(DEFAULT_LAT_INITIAL_CASE, DEFAULT_LNG_INITIAL_CASE)

        private const val DEFAULT_LAT = 20.744081
        private const val DEFAULT_LNG = -42.991302
        private val DEFAULT_POSITION = Position(DEFAULT_LAT, DEFAULT_LNG)
    ...
    fun `initial case`() = testCoroutineRule.runTest {
        // WHEN
        every { getAllRealEstatesUseCase.invoke() } returns flowOf(listOf())
        every { currentPositionRepository.getCurrentPositionFlow() } returns flowOf(DEFAULT_POSITION_INITIAL_CASE)

        mapViewModel.mapViewStateLiveData.observeForTesting(this) {
            // THEN
            assertThat(it.value).isEqualTo(getMapViewStateInitialCase())

            verify {
                getAllRealEstatesUseCase.invoke()
                currentPositionRepository.getCurrentPositionFlow()
            }

            confirmVerified(getAllRealEstatesUseCase, currentPositionRepository)
        }
    }
    ...
    fun `nominal case`() = testCoroutineRule.runTest {
        ...
        every { currentPositionRepository.getCurrentPositionFlow() } returns flowOf(DEFAULT_POSITION)

        mapViewModel.mapViewStateLiveData.observeForTesting(this) {
            // THEN
            assertThat(it.value).isEqualTo(getMapViewState())
    ...
    // MAP VIEW STATE
    private fun getMapViewStateInitialCase(): MapViewState {
        return MapViewState(0L, "", DEFAULT_LAT_INITIAL_CASE, DEFAULT_LNG_INITIAL_CASE, R.drawable.ic_custom_google_marker_red, listOf())
    }

    private fun getMapViewState(): MapViewState {
        return MapViewState(0L, "", DEFAULT_LAT, DEFAULT_LNG, R.drawable.ic_custom_google_marker_red, listOfPositions())
    }

    private fun listOfPositions(): List<Position> {
        return listOf(Position(FIRST_DEFAULT_LAT, FIRST_DEFAULT_LONG), Position(SECOND_DEFAULT_LAT, SECOND_DEFAULT_LONG))
    }
